"""The rules Prometheus is not allowed to break (Phase 7, §7.13).
...
from application.prometheus.delegation import CapabilityDelegator, manager_actor
...
# --- No employee is named in Prometheus's code (§7.6, and the phase's DoD) ------------


def test_prometheus_never_names_a_declared_employee() -> None:
    ...
    assert not offenders, f"Prometheus names a concrete employee: {offenders}"


def test_prometheus_reaches_the_workforce_only_through_the_registry() -> None:
    """No file access, no YAML, no directory walk, and no runtime.

    The runtime one matters as much as the rest: Prometheus holds `TaskExecution`, a
    ...
    for path in sorted(PROMETHEUS_DIR.rglob("*.py")):
        ...
        # `application.task_runner` is in Prometheus's own layer, so the layering
        ...
        if found:
            offenders[str(path.relative_to(REPO_ROOT))] = found
    assert not offenders, f"Prometheus reached around its contracts: {offenders}"


# --- Delegation never escalates privileges (§7.13) ----------------------------


async def test_prometheus_cannot_grant_a_tool_the_employee_does_not_have() -> None:
...
def test_the_manager_actor_can_never_exceed_the_union_of_the_workforce() -> None:
    ...
    prometheus = manager_actor([first, second])

    assert prometheus.actor_kind is ActorKind.PROMETHEUS
    assert prometheus.allowed_tools == frozenset({"fs.read", "web.search"})
    assert "*" not in prometheus.allowed_tools, "a wildcard would make the intersection meaningless"
    # And the intersection with any one employee is that employee's own rights.
    assert effective_tools(prometheus, first) == first.allowed_tools
    assert effective_tools(prometheus, second) == second.allowed_tools
...
# --- Prometheus never approves its own work (§7.10) ----------------------------------


def test_prometheus_never_resolves_an_approval() -> None:
    """Asking is the manager's; answering is a person's.

    Checked as a property of the source rather than of one run: an approval Prometheus
    ...
    for path in sorted(PROMETHEUS_DIR.rglob("*.py")):
        ...
        if found:
            offenders[str(path.relative_to(REPO_ROOT))] = found
    assert not offenders, f"Prometheus touched the approval machinery: {offenders}"


async def test_a_delegated_task_still_stops_at_the_gate() -> None:
    """Delegation changes who asked, not whether the gate applies.

    An irreversible action inside a task Prometheus handed out is refused exactly as it
