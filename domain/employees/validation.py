...
are known - the container at start-up, `prometheus employees`, a test - without any of
...
def _unbacked_capabilities(
    ...
    An error, because it is false on every machine: Prometheus searches by these, so an
...
def _undeclared_capabilities(
    ...
    The quiet one. The employee can do the work and Prometheus will never send it any,
